...
simulation.3.test <- function(prev,n.sampled,Se,Sp) {
  ...
  test.results <- list(
  ppp = sum(test.1 & test.2 & test.3),
  ppn = sum(test.1 & test.2 & !test.3),
  pnp = sum(test.1 & !test.2 & test.3),
  pnn = sum(test.1 & !test.2 & !test.3),
  npp = sum(!test.1 & test.2 & test.3),
  npn = sum(!test.1 & test.2 & !test.3),
  nnp = sum(!test.1 & !test.2 & test.3),
  nnn = sum(!test.1 & !test.2 & !test.3))
  return(test.results)
}
```
